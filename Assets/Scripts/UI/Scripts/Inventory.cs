...
public class Inventory 
{
    ...
    public Inventory(uint size = Defualt_Size)
    {
        ...
        itemDataManager = GameManager.Instance.ItemData; //아이템 데이터에 따른 처리 필요
    }
        ...
        if ((from != to) && IsValidIndex(from) && IsValidIndex(to))
        {
            ...
            if (!fromSlot.IsEmpty)
            {
                ...
                if (to == tempSlotIndex)
                {
                    ...
                    TempSlot.SetFromIndex(fromSlot.Index);  // 인덱스 저장(FromIndex)
                }
                else
                {
                ...
                if (fromIsTemp)
                {
                    ...
                    if (returnSlot.IsEmpty)
                    {
                        ...
                        returnSlot.AssignSlotItem(toSlot.ItemData, toSlot.IsEquipped);
                        toSlot.AssignSlotItem(TempSlot.ItemData, TempSlot.IsEquipped);
