...
public class 가짜가짜 : MonoBehaviour
{
    [Tooltip("변수들")]
    ...
    private void Awake()
    {
        ...
        road = transform.GetChild(2).gameObject;
    }
    ...
    void MapLoad()
    {
        ...
        for (int i = 0; i < rooms.Count; i++)
        {
            ...
            {
                int random = Random.Range(0, mapCount);    //<-- 여기도         

                sceneLoadState[random] = SceneLoadState.Loaded;

                AsyncOperation async = SceneManager.LoadSceneAsync(sceneNames[random], LoadSceneMode.Additive);
                ...
                async.completed += (_) =>
                {
                    Scene loadedScene = SceneManager.GetSceneByName(sceneNames[random]);
                    if (i < mapCount)
                    {
